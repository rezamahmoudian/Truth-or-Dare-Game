...
## ۵.۱ ظرفیت و تعداد worker

سقف پیام‌های همزمان را **تعداد worker** تعیین می‌کند، نه رم و نه CPU.

دلیلش در معماری Channels است: `database_sync_to_async` همه‌ی کار دیتابیس یک
پروسه را روی **یک ترد مشترک** اجرا می‌کند. پس هر worker پیام‌ها را یکی‌یکی
می‌نویسد و ظرفیتش برابر است با `۱ ثانیه ÷ زمان نوشتن یک پیام`.

اندازه‌گیری شده روی همین کد (`scripts/loadtest_ws.py`):

| worker | ۴ روم همزمان | ۸ روم همزمان |
|---|---|---|
| ۱ | p50 ‏۹۶۶ms | p50 ‏۲۹۶۳ms و ۳۵٪ پیام‌ها نرسید |
| ۴ | p50 ‏۷۷ms | p50 ‏۱۸۲ms، همه رسید |

هر worker تقریباً **۲۰ تا ۲۵ پیام در ثانیه**؛ روی سروری با SSD واقعی حدود دو
برابر، چون نیمی از هزینه‌ی هر پیام `fsync` دیسک است (۲۴ms با `fsync`، ۱۲ms
بدون آن).

سوکت‌های **بی‌کار** تقریباً رایگان‌اند — ۵۰۰ اتصال باز هیچ اثری روی تأخیر
نداشت. چیزی که هزینه دارد، پیام است نه اتصال.

اندازه‌گیری روی سرور خودتان:

```bash
docker compose --env-file .env.production -f docker-compose.yml -f docker-compose.prod.yml exec backend python scripts/loadtest_ws.py --idle 300 --rooms 12
```

اگر `p95` از یک ثانیه گذشت یا پیامی نرسید، `UVICORN_WORKERS` را بالا ببرید.
این اسکریپت کاربر و روم واقعی می‌سازد؛ **روی دیتابیس پروداکشنِ شلوغ اجرایش
نکنید.**
